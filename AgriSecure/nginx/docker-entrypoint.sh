...
# SÃ©lectionner la configuration nginx selon NGINX_MODE
if [ "$NGINX_MODE" = "dev" ]; then
    ...
    if [ -f /etc/nginx/nginx.dev.conf ]; then
        ...
        echo "âœ“ Copied nginx.dev.conf -> nginx.conf"
    else
        echo "âœ— ERROR: nginx.dev.conf not found!"
        exit 1
    fi
    ...
    if [ -f /etc/nginx/nginx.prod.conf ]; then
        ...
        echo "âœ“ Copied nginx.prod.conf -> nginx.conf"
    else
        echo "âš  WARNING: nginx.prod.conf not found, using default nginx.conf"
    fi

else
    echo "âš  NGINX_MODE not set, using default configuration (dev mode)"
    if [ -f /etc/nginx/nginx.dev.conf ]; then
        cp /etc/nginx/nginx.dev.conf /etc/nginx/nginx.conf
        echo "âœ“ Defaulting to nginx.dev.conf"
    fi
fi
